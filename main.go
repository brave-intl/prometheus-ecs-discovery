...
func GetClusters(svc *ecs.ECS) (*ecs.ListClustersOutput, error) {
	...
	for {
		req, myoutput := svc.ListClustersRequest(input)
		err := req.Send()
		if err != nil {
			return nil, err
		}
...
func (t *AugmentedTask) ExporterInformation() []*PrometheusTaskInfo {
	...
	for _, i := range t.Containers {
		...
		if d.DockerLabels[*prometheusJobNameLabel] == nil {
			continue
		}
...
func AddTaskDefinitionsOfTasks(svc *ecs.ECS, taskList []*AugmentedTask) ([]*AugmentedTask, error) {
	...
	for w := 1; w <= 4; w++ {
		go func() {
			for in := range jobs {
				req, out := svc.DescribeTaskDefinitionRequest(in)
				err := req.Send()
				results <- struct {
					out *ecs.DescribeTaskDefinitionOutput
					err error
...
func DescribeInstancesUnpaginated(svc *ec2.EC2, instanceIds []string) ([]ec2.Instance, error) {
	...
	for _, chunkedInstanceIds := range splittedInstanceIds {
		input := &ec2.DescribeInstancesInput{
			InstanceIds: StringToStarString(chunkedInstanceIds),
		}
		for {
			req, output := svc.DescribeInstancesRequest(input)
			err := req.Send()
			if err != nil {
				return nil, err
			}
	...
	for clusterArn, containerInstancesArns := range clusterArnToContainerInstancesArns {
		...
		for _, chunkedKeys := range splittedKeys {
			input := &ecs.DescribeContainerInstancesInput{
				Cluster:            &clusterArn,
				ContainerInstances: StringToStarString(chunkedKeys),
			}
			req, output := svc.DescribeContainerInstancesRequest(input)
			err := req.Send()
			if err != nil {
				return nil, err
			}
	...
	for w := 1; w <= 4; w++ {
		go func() {
			for clusterArn := range jobs {
				...
				var err error
				for {
					req, output := svc.ListTasksRequest(input)
					err1 := req.Send()
					if err1 != nil {
						err = err1
					...
					reqDescribe, descOutput := svc.DescribeTasksRequest(&ecs.DescribeTasksInput{
						Cluster: clusterArn,
						Tasks:   output.TaskArns,
					})
					err2 := reqDescribe.Send()
					if err2 != nil {
						err = err2
...
func main() {
	...
	work := func() {
		...
		if *cluster != "" {
			req, res := svc.DescribeClustersRequest(&ecs.DescribeClustersInput{
